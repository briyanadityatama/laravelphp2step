@extends('laravelphp2step::layouts.app')

@section('title')
    {{ trans('laravelphp2step::laravel-verification.title') }}
@endsection
        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel verification-form-panel">
                <div class="panel-heading text-center" id="verification_status_title">
                    ...
                    <p class="text-center">
                        <em>
                            {{ trans('laravelphp2step::laravel-verification.subtitle') }}
                        </em>
                    </p>
                </div>
                            ...
                            <div class="col-xs-3">
                                <div class="{{ $errors->has('v_input_1') ? ' has-error' : '' }}">
                                    <label for="v_input_1" class="sr-only control-label">
                                        {{ trans('laravelphp2step::laravel-verification.inputAlt1') }}
                                    </label>
                            ...
                            <div class="col-xs-3">
                                <div class="{{ $errors->has('v_input_2') ? ' has-error' : '' }}">
                                    <label for="v_input_2" class="sr-only control-label">
                                        {{ trans('laravelphp2step::laravel-verification.inputAlt2') }}
                                    </label>
                            ...
                            <div class="col-xs-3">
                                <div class="{{ $errors->has('v_input_3') ? ' has-error' : '' }}">
                                    <label for="v_input_3" class="sr-only control-label">
                                        {{ trans('laravelphp2step::laravel-verification.inputAlt3') }}
                                    </label>
                            ...
                            <div class="col-xs-3">
                                <div class="{{ $errors->has('v_input_4') ? ' has-error' : '' }}">
                                    <label for="v_input_4" class="sr-only control-label">
                                        {{ trans('laravelphp2step::laravel-verification.inputAlt4') }}
                                    </label>
                            ...
                            <div class="row">
                                <div class="col-xs-8 col-xs-offset-2 text-center submit-container">
                                    <button type="submit" class="btn btn-lg btn-{{ $remainingAttemptsClass }} btn-block" id="submit_verification" tabindex="5">
                                        ...
                                        {{ trans('laravelphp2step::laravel-verification.verifyButton') }}
                                    </button>
                                </div>
                                <div class="col-xs-12 text-center">
                                    <p class="text-{{ $remainingAttemptsClass }}">
                                        <small>
                                            <span id="remaining_attempts">{{ $remainingAttempts }}</span> {{ trans_choice('laravelphp2step::laravel-verification.attemptsRemaining', $remainingAttempts) }}
                                        </small>
                                    </p>
                                </div>
                                <div class="col-xs-12 text-center">
                                    <a class="btn btn-link" id="resend_code_trigger" href="#" tabindex="6">
                                        {{ trans('laravelphp2step::laravel-verification.missingCode') }}
                                    </a>
                                </div>
                            </div>
                        </div>
...
@section('foot')
    ...
    @php
        $minutesToExpire = config('laravelphp2step.laravelphp2stepExceededCountdownMinutes');
        $hoursToExpire = $minutesToExpire / 60;
    @endphp

    <script type="text/javascript" src="https://code.jquery.com/jquery-3.2.1.min.js"></script>

    @include('laravelphp2step::scripts.input-parsing-auto-stepper');
                    ...
                    if (response.status === 418) {
                        ...
                        switch(remainingAttempts) {
                            case 0:
                                ...
                                swal(
                                  "{{ trans('laravelphp2step::laravel-verification.verificationLockedTitle') }}",
                                  "{{ trans('laravelphp2step::laravel-verification.verificationLockedMessage') }}",
                                  'error'
                                );
                                break;
                            ...
                            case 1:
                                submitTrigger.addClass('btn-danger');
                                swal(
                                  "{{ trans('laravelphp2step::laravel-verification.verificationWarningTitle') }}",
                                  "{{ trans('laravelphp2step::laravel-verification.verificationWarningMessage', ['hours' => $hoursToExpire, 'minutes' => $minutesToExpire,]) }}",
                                  'error'
                                );
                                break;
                        ...
                        if (remainingAttempts === 0) {
                            $('#verification_status_title').html('<h3>{{ trans('laravelphp2step::laravel-verification.titleFailed') }}</h3>');
        ...
        $("#resend_code_trigger").click(function(event) {
            ...
            function swalCallback(title, message, status) {
                swal({
                    ...
                    confirmButtonText: "{{ trans('laravelphp2step::laravel-verification.verificationModalConfBtn') }}",
                });
