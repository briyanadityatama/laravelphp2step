...
namespace briyanadityatama\laravelphp2step\App\Traits;
...
trait LaravelPHP2StepTrait
{
    ...
    {
        $expireMinutes = config('laravelphp2step.laravelphp2stepVerifiedLifetimeMinutes');
        $now = Carbon::now();
    ...
    {
        $tomorrow = Carbon::parse($time)->addMinutes(config('laravelphp2step.laravelphp2stepExceededCountdownMinutes'))->format('l, F jS Y h:i:sa');
        $remaining = $time->addMinutes(config('laravelphp2step.laravelphp2stepExceededCountdownMinutes'))->diffForHumans(null, true);
    ...
    {
        $now = Carbon::now();
        $expire = Carbon::parse($time)->addMinutes(config('laravelphp2step.laravelphp2stepExceededCountdownMinutes'));
        $expired = $now->gt($expire);
